...
const HUD: React.FC<HUDProps> = ({ currentZone, onNavigate, onPing }) => {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-50 flex flex-col justify-end">
      ...
      <div className="absolute top-1/2 -translate-y-1/2 right-4 md:right-8 flex flex-col gap-2 pointer-events-auto">
        <div className="bg-slate-50/95 backdrop-blur-md border border-slate-200 rounded-xl p-2 shadow-xl flex flex-col gap-1">
             ...
             {Object.values(CELESTIAL_DATA).map((data) => (
                ...
                >
                    ...
                    <span className="absolute right-full mr-3 bg-white border border-slate-300 text-slate-800 text-xs font-mono px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none backdrop-blur-sm shadow-xl font-bold">
      ...
      <div className="w-full pointer-events-auto">
        ...
        <div className="max-w-4xl mx-auto mb-4 flex justify-center md:justify-end gap-3 px-4">
            <button 
                onClick={onPing}
                disabled={currentZone === CelestialId.EARTH}
                className={`
                    flex items-center gap-2 px-4 py-2 bg-white/95 backdrop-blur border rounded-lg transition-all text-sm font-mono uppercase tracking-wider shadow-lg font-semibold
        ...
        <div className="bg-slate-50/95 border-t border-slate-300 backdrop-blur-xl p-6 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] max-h-[40vh] overflow-y-auto">
            <div className="max-w-5xl mx-auto font-mono text-sm md:text-base">
                
                {/* JSON Representation - High Contrast Text */}
                <div className="bg-white rounded-xl border border-slate-300 p-4 md:p-6 relative overflow-hidden group shadow-sm">
